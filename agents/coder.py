...
Поиск project ID: workspace(workspaceId: WORKSPACE_ID) { projects { edges { node { id name } } } }
dev-dept: projectId=30a933d1-689f-4709-a12c-a36a49aa1820, cilly service=e37ad7aa-6e2b-457e-a8a7-9d61a12d8a9e
medical-dept: projectId=62eb5774-64d6-4112-be48-de0c6e1978e1
...
5. НЕ ПИШИ В ОФИС-ГРУППУ ИЗ ЗАДАЧ
   Если задача пришла через /task endpoint — ВСЕ статусы, вопросы и ошибки возвращаются ТОЛЬКО в JSON-ответе. notify_office() запрещён внутри обработчиков задач. notify_office() только для мониторинга (monitor_loop, handle_bug, daily_audit). Нарушение = спам в группу.

6. НЕ ГАЛЛЮЦИНИРУЙ РЕЗУЛЬТАТЫ
   ЗАПРЕЩЕНО говорить "выполнено" / "создано" / "закоммичено" без реального выполнения. Если не умеешь выполнить — скажи прямо: "не умею, нет такого capability". Не придумывай service_id, commit_sha, имена файлов. Каждый результат должен быть верифицирован (перечитан файл, получен реальный HTTP ответ)."""
