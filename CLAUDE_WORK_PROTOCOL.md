...
## 0. Главное правило — делегируй Силли

> **Всё что может сделать Силли — делает Силли. Не ты.**

Силли — кодер-агент с прямым доступом к Railway, GitHub, Redis и Telegram.
Она существует именно для того чтобы делать реальную работу, а не ты.

**Что делегировать Силли (всё из этого списка — через /task):**
- Любой GitHub push / чтение файла
- Любой деплой или перезапуск сервиса
- Redis запросы и аудиты
- Исправление бага в боте
- Создание или обновление бота
- Отправка сообщений в Telegram группы

**Как давать задачу:**
```
POST https://cilly-bot-production.up.railway.app/task
{"message": "конкретная задача пошагово", "user_id": 391077101}
```

**Если Силли не справляется или не понимает:**
1. Проверь intent — возможно нужного action нет в INTENT_PROMPT (см. раздел 3)
2. Перефразируй задачу чётче — Силли работает лучше на конкретных командах чем на описаниях
3. Разбей на атомарные шаги и давай по одному — каждый fix_bot должен содержать ОДНУ замену (old→new) в ОДНОМ файле. Никогда не давай fix_bot с несколькими заменами или в нескольких файлах сразу.
4. Если не умеет — **учи**: добавь intent в `agents/coder.py` → `INTENT_PROMPT` + handler

**Ты (Claude) делаешь сам только если:**
- Силли недоступна (проверь /health)
- Задача требует рассуждений или архитектурных решений (не механических действий)
- Нужно срочно, а Силли медленно отвечает

**Принцип:** чем больше Силли делает — тем меньше токенов тратится и тем умнее она становится.
Каждый раз когда ты делаешь за неё — ты платишь дважды и ничему её не учишь.
