...
- `publish_pending_lessons()` — единственный публикатор: постит только уроки без флага, ставит флаг и **коммитит** `lessons.json`. Идемпотентно, переживает сброс Redis, не может зафлудить (анти-флуд лимит 25/раз).
- **Силли подтягивает новые уроки сама на аудите** (`run_daily_audit` → `publish_pending_lessons`). Влад/Клод просто дописывают урок в `lessons.json` (durable-история).
...
- `cleanup_group`: добавлен режим **`all_bots`** (слова «всё/вычистить/wipe/чистый лист») — полная зачистка сообщений ботов в Bug Lessons.
