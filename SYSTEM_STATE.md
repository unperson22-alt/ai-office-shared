...
Канонический список людей — `shared.identity.PEOPLE`.
...
🔴 **`675773302` — неопознанный доступ.** Он сидит в `ALLOWED_USERS` у kriss-bot
и до 13.08 числился там Луком. Обе привязки оказались ложными: Лук — это
`331989769`, и Криссом он не пользуется. То есть это неизвестный аккаунт с
доступом к личному ассистенту Влада. Ждёт решения владельца: назвать или убрать.
Удалить теперь безопаснее, чем раньше — отказ виден в логах
(`access_denied`), так что живой человек упрётся в стену заметно, а не молча.
