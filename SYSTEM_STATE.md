...
⁶ Подтверждено Владом 2026-08-12. Канонический список людей — `shared.identity.PEOPLE`.

**Лук не пользуется Криссом** (Влад, 13.08): Крисс — личный ассистент Влада,
Лук общается с Гослингом и Филли. В коде Крисса Лука нет и не было; связка
жила только в документации.

✅ Вайтлист Крисса вычищен 2026-08-13: неопознанный ID удалён из `ALLOWED_USERS`
по решению владельца. Сейчас там Влад и Яна (текущий аккаунт `zefiiry` плюс её
прежний). Лук в списке отсутствует намеренно — Криссом он не пользуется.

### 🚨 Исходящий вызов обязан читать код ответа

`await client.post(...)` в asyncio возвращается нормально при ЛЮБОМ статусе —
401 и 500 в том числе; исключение бросает только транспорт. То есть поведение
по умолчанию самой частой строки в этом коде — считать отказ успехом.

Правило: **каждый вызов через границу сервиса читает `status_code` и пишет
неудачу туда, куда смотрит человек** (`office:logs`, не только stdout).

И отдельно: **не записывай успех до того, как он подтверждён**. Болталка
добавляла бота в список «позвали» независимо от ответа — при 401 лог сообщил
бы «позвал МИЛЛИ», хотя реплики не будет. Ложный успех хуже молчания: молчание
оставляет искать, ложный успех отправляет искать не туда.

⚠️ Перед включением `OFFICE_RPC_STRICT=1` проверить непроверяющих вызывающих:
дата включения — это дата, когда они все сломаются разом. Инвентарь на
15.08 — 36 мест в 19 репо (урок #98), исправлены 4 самых опасных.

### 🔑 Доступ к ботам живёт в env, а не в коде

`ALLOWED_USERS` — Railway-переменная сервиса (kriss-bot, mama-bot, billy-bot,
family-dept/elliss), НЕ константа в репозитории. Grep по коду не найдёт ни одного
ID и создаёт ложное впечатление, что вайтлист пуст. `identity.PEOPLE` — только про
то, как боты НАЗЫВАЮТ человека; доступа он не выдаёт.

Отказ по вайтлисту с 2026-08-12 пишет `access_denied` в `office:logs`
(`shared.auth.note_denied_access`), а Крисс отвечает незнакомцу на `/start`.
До этого гейт делал голый `return`, и «нет в списке» выглядело как «бот умер»
(урок #92).

### ⚠️ «Not Authorized» от Railway ≠ протухший токен

Дважды подряд этот ответ диагностировали как истёкший токен, и оба раза зря.
Проверено 2026-08-12: токен Силли рабочий, `{ projects { … } }` и
`deployments(input:{serviceId:…})` отвечают штатно. При этом `{ me { id } }`
даёт «Not Authorized» — это командный токен, а `me` резолвит пользователя,
которого у него нет. Историю см. также 2026-07-03: там «Not Authorized» был
из-за галлюцинированного serviceId в промпте, тоже не из-за токена.

**Правило:** прежде чем объявлять токен мёртвым, прогони запрос из рабочего
скоупа (`deployments` по известному serviceId). Один провалившийся probe —
не диагноз всей системы.
...
**Как спрашивать логи — `GET /logs` у Силли** (с 2026-08-16, урок #101)

Не через `/task`: там ответ режется на 3000 символах и приходит сырой дамп
вместо ответа. Ручка возвращает сводку — сколько, когда, с каким тактом.

```
curl -H "X-Office-Token: $OFFICE_RPC_TOKEN" \
  "https://ai-office-shared-production.up.railway.app/logs?bot=девви&days=1"
→ девви: 14 записей | 04:07:52–04:20:27 | чаще всего task_received ×7, каждые ~2м01с
```

|Параметр|По умолчанию|Что делает|
|--------|-----------|----------|
|`bot`   |все        |Имя берётся из ключей Redis, а не из `identity.BOTS` — воркеров dev-отдела (девви, рикки, тести, секки, скрибби) в реестре нет|
|`days`  |1          |Последние N суток UTC, потолок 30|
|`event` |—          |Фильтр по имени события|
|`level` |—          |`info` / `warn` / `error`|
|`limit` |0          |Сколько записей приложить к сводке; 0 — только сводка, потолок 200|
|`format`|`text`     |`json` — вся структура со сводкой и разбивкой|

`cadence_sec` в сводке — **медианный** интервал между повторами самого частого
события. Именно этим числом «бот поработал восемь раз за день» отличается от
«бот молотит одно и то же каждые две минуты». Медиана, а не среднее: одна
долгая пауза не должна маскировать ровный такт.
...
|watchdog-bot|Railway-сервис (`watchdog.py`)        |**Кто чинит Силли.** Сам опрашивает её `/health` каждые 120 с, после 2 фейлов подряд редеплоит, при неудаче откатывает на последний хороший деплой. HTTP-ручек нет — просить его не нужно и нельзя. Остальные боты чинит Силли; Силлю — только он. Инцидент 16.08: Филли слала «передеплой силли» на адрес самой Силли, пока та лежала (урок #99).|
...
|`BUG-006` |watchdog-bot / Силли |`SILLI_SERVICE_ID` = `efa6bd21-91d8-467f-8250-60f8a3853791` — сервис на Railway помечен `deletedAt = 2026-05-30`, `serviceInstances` пуст, деплоев ноль. Вачдог целится в удалённую запись, то есть автоподъём лидера не работает с конца мая; вручную Силлю поднимает только автодеплой Railway по коммиту в main. Настоящий id коду недоступен — сервис в проекте, которого токен Силли не видит.|🔴 Открыт 2026-08-16 — **нужен Влад**: Railway UI → сервис `ai-office-shared` → id → `SILLI_SERVICE_ID` (watchdog-bot) и `SELF_SERVICE_ID` (Силли)|
...
| LUK_USER_ID: правка ID Лука | ↩️ ОТМЕНЕНО 2026-08-12 — правка была ошибочной. Код всегда держал `331989769`, и Влад подтвердил, что это верный ID Лука. Тогда поправили документ, а не код, и рассинхрон жил дальше. Не применять снова. |
...
## Публикация уроков в Bug Lessons — durable-механизм (2026-06-20)

**Инцидент:** команда «запость новые уроки» вывалила в Bug Lessons разом все 52 урока. Корень: `post_lessons` дедупил по числовым id из Redis-сета `office:lessons:posted_ids`, но сет их не содержал (уроки заводились через git/PR и `post_lesson`, который писал в тот же сет title-hash, а не id). Два источника правды (состояние в Redis, уроки в git) → флуд. См. урок #53.

**Новый контракт (единый источник правды — git):**
- В каждом уроке `lessons/lessons.json` — флаг **`posted_to_group`** (bool) + `posted_at`. Это и есть состояние «опубликован».
- `publish_pending_lessons()` — единственный публикатор: постит только уроки без флага, ставит флаг и **коммитит** `lessons.json`. Идемпотентно, переживает сброс Redis (лимит 100/проход).
- **Силли подтягивает новые уроки сама на аудите** (`run_daily_audit` → `publish_pending_lessons`) и на старте (`publish_pending_on_startup`, без удалений). Влад/Клод просто дописывают урок в `lessons.json` (durable-история).
- `post_lessons` (интент) и `add_lessons` (endpoint) теперь зовут `publish_pending_lessons`; Redis-ключ `office:lessons:posted_ids` **больше не используется** (deprecated).

**Чистка Bug Lessons — только дедуп (2026-06-20, после инцидента, см. урок #54):**
- Авто-чистка-wipe **убрана**: режим `all_bots` и старт-задача `bug_lessons_migration_task` (полное удаление сообщений ботов) удалены из кода. Деструктив по умолчанию + автозапуск однажды снёс все 76 сообщений уроков вместо дублей.
- Единственный режим чистки — **`dedup_lessons`** (`cleanup_group`, по явной команде «дубли/повтор»): оставляет по одному сообщению на «Урок #N», удаляет повторы. Уникальные уроки не трогает.
- **Восстановление**: сообщения уроков durable в `lessons.json` → удалённые возвращаются перепостом (снять `posted_to_group` у нужных id → `publish_pending_lessons`). Так вернули #1–#51 после инцидента.
