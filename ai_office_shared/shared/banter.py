...
def is_banter(task_data: dict) -> bool:
    ...
    Пустой payload не роняет: `None` и `{}` — это просто «не болталка».
    Раньше на None была AttributeError, и вызывающему пришлось бы страховать
    вызов у себя — то есть помнить про это в семи ботах вместо одного места.
    """
    return str((task_data or {}).get("source", "")).upper() == "BANTER"


def should_post_to_group(task_data: dict) -> bool:
    """
    Постить ли ответ в офис-группу. ОДНО правило для всех семи ботов.

    Правил было четыре, и это выяснилось по живой ленте 14.09.2026:
        безусловно ................ Билли, Крисс, Вилли, Гослинг
        is_banter or notify ....... Милли, Тилли
        ... or source not in (...) . Доктор

    Чем это плохо, помимо разнобоя. Филли на пути «бот→Филли→бот» кладёт
    ответ агента в ленту сама — потому что на этом пути ответ уходит в ЛИЧКУ
    через /reply, а не в группу. Четверо безусловных при этом всё равно
    постили в группу и писали ленту ещё раз: строка задваивалась, а личный
    разговор протекал в общий чат.

    Правило: постим, если это болталка ИЛИ вызывающий явно попросил
    (`notify`). Умолчание — НЕ постить: тот, кто хочет реплику в группе,
    обязан это сказать. Так вызывающий знает, кто пишет в ленту, и ему не
    нужно помнить наизусть привычки каждого бота.
    """
    return is_banter(task_data) or bool((task_data or {}).get("notify", False))


def depth_of(task_data: dict) -> int:
    """Глубина нити из payload /task. 0 — обычный запрос, не болталка."""
    try:
        return int((task_data or {}).get("depth", 0) or 0)
