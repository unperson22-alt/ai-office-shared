...
async def post_to_group(
    *,
    ...
    # «Не подписывайся своим именем — его подставят за тебя» стоит в промте
    # болталки с самого начала, и выполняется не всегда: 13.09.2026 Крисс
    # написала в группу «Крис:\nГослинг, "на месте" — это диагноз или жизненная
    # позиция?», а Telegram и так показывает её имя над сообщением. Выходит
    # подпись дважды.
    #
    # У Крисс источником был не промт, а её собственный код — send_to_group
    # звали с f"Крис:\n{response}", — но чинить только его значит оставить
    # второй источник: модель подписывается сама, когда ей вздумается. Поэтому
    # правило живёт здесь, на ОБЩЕМ пути в группу, а не в семи промтах.
    #
    # Импорт локальный намеренно: banter тянет identity и httpx, а group_post
    # зовут из кода отправки — модулю уровня транспорта незачем поднимать
    # болталку при импорте.
    if sender_name:
        from .banter import strip_self_prefix
        text = strip_self_prefix(text, sender_name)
