...
PEOPLE: dict[str, dict] = {
    ...
    "лук": {
        ...
        "role":    "друг Влада со школы, свой человек в офис-чате. С Криссом НЕ "
                   "взаимодействует и доступа к нему не имеет — Крисс личный "
                   "ассистент Влада. Лук общается с Гослингом и Филли.",
    },
    "ангелина": {
        "display": "Ангелина",
        "user_id": 7354462052,
...
# ⚠️ ЭТОТ РЕЕСТР НЕ ДАЁТ ДОСТУПА. Кто может писать боту, решает Railway-переменная
# ALLOWED_USERS у kriss-bot / mama-bot / billy-bot / family-dept-elliss. PEOPLE
# влияет только на то, как боты НАЗЫВАЮТ человека (roster_prompt). Добавить
# кого-то сюда и ждать, что он сможет писать Криссу, — не сработает: нужен
# отдельный шаг в env (инцидент 2026-08-12 с новым аккаунтом Яны).
...
# ── Кто пишет прямо сейчас ─────────────────────────────────────────────────────

def speaker_prompt(user_id, first_name: str = "") -> str:
    """
    Блок системного промта «тебе пишет вот этот человек».

    ПРОБЛЕМА, которую решаем (инцидент 2026-08-21):
        Яна написала Криссу «прибери недоліки на обличчі», Крисс ответил
        «Влад, я розумію задачу...». В промте у него был ростер офиса, где
        Влад — владелец, и роль «личный ассистент Влада». Кто пишет ИМЕННО
        СЕЙЧАС, в промт не попадало вообще, и модель подставляла самого
        вероятного человека. Реестр PEOPLE знал про Яну — но знание, которое
        не доехало до промта, ничего не значит.
    РЕШЕНИЕ:
        Одна строка фактом, до всякого рассуждения: вот user_id, вот имя.
        roster_prompt() отвечает на вопрос «кто есть кто», этот блок — на
        вопрос «с кем ты сейчас разговариваешь». Второй без первого работает,
        первый без второго — нет.

    Args:
        user_id: Telegram id автора сообщения.
        first_name: как человек подписан в Telegram — нужен, когда id не в
                    реестре: назвать по имени из профиля лучше, чем гадать.

    Returns:
        Готовый блок для системного промта. Пустая строка — только если не
        известно вообще ничего (нечего утверждать — молчим, а не выдумываем).
    """
    canon = person(user_id) if user_id else None
    if canon:
        meta = PEOPLE[canon]
        name = meta["display"]
        role = meta.get("role", "")
        others = [m["display"] for k, m in PEOPLE.items() if k != canon]
        lines = [
            "== КТО ПИШЕТ ПРЯМО СЕЙЧАС ==",
            f"Тебе пишет {name} (Telegram id {meta['user_id']}). "
            f"{role[:1].upper()}{role[1:]}",
            f"Обращайся по имени — {name}. Автор всех сообщений в этом "
            f"диалоге: {name}.",
            f"Это НЕ {', НЕ '.join(others)}. Чужим именем не называй никогда.",
        ]
        return "\n".join(lines)
    ...
    return "\n".join([
        "== КТО ПИШЕТ ПРЯМО СЕЙЧАС ==",
        f"Тебе пишет человек, подписанный в Telegram как «{name}» — его нет в "
        "реестре офиса.",
        f"Обращайся к нему «{name}» или без имени. Не подставляй вместо него "
        "Влада или кого-то ещё из офиса.",
    ])
