...
Доступные модули:
    ...
    group_post   — post_to_group() → PostResult, log_delivery(): отправка в
                   офис-группу, которая не врёт. Прежний `send_to_group` был
                   скопирован в шесть ботов и имел три молчаливых выхода, два
                   без следа вовсе (незаданный OFFICE_CHAT_ID и ok:false от
                   Telegram), а вызов возвращаемое значение выбрасывал и писал
                   MSG_OUT безусловно. 10.09.2026 Милли и Доктор отчитались за
                   реплики, которых в группе не было (инварианты №4, №5, №8)
