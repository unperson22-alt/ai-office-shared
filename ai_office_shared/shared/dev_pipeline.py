...
# ── Сбой провайдера, который не лечится повтором ──────────────────────────────
# 23.08.2026 две заявки подряд (62ffa25b5e30, aa573bddb6d7) сожгли ~70 минут и
# около 19 вызовов Девви, подтверждая одну и ту же ошибку: HTTP 400 «Your credit
# balance is too low to access the Anthropic API». Повтор здесь ничего не меняет
# по определению: у аккаунта нет денег, и он не появятся оттого, что мы спросим
# ещё раз. Хуже того, владельцу это докладывалось как «команда не дала код» —
# то есть как провал отдела, хотя отдел не смог даже начать.
#
# Отличать обязательно от 429 и 5xx: те как раз ретраить нужно, они проходят.

_UNRECOVERABLE = (
    ("credit balance",        "на счету Anthropic закончились деньги"),
    ("invalid_request_error", "провайдер отклонил сам запрос (invalid_request_error)"),
    ("authentication_error",  "ключ API не принят (authentication_error)"),
    ("permission_error",      "ключу API не хватает прав (permission_error)"),
    ("invalid x-api-key",     "ключ API неверный"),
    ("error code: 400",       "провайдер ответил 400"),
    ("error code: 401",       "провайдер ответил 401 — ключ не принят"),
    ("error code: 403",       "провайдер ответил 403 — доступ запрещён"),
)


def unrecoverable_provider_error(*texts) -> str:
    """
    Короткая причина, если в ответах воркеров виден невосстановимый сбой доступа
    к LLM. Пусто — ничего такого нет.

    Возвращаем ЧЕЛОВЕЧЕСКУЮ причину, а не True: она уходит владельцу, и «это не
    код, а деньги на счету» он должен прочитать словами, а не вывести из кода
    ошибки. Проверка идёт по подстроке в тексте, потому что ошибка приезжает уже
    завёрнутой воркером в его «ERROR: …», а не как исключение с полями.
    """
    blob = " ".join(str(t or "") for t in texts).lower()
    if not blob.strip():
        return ""
    # 429 и 5xx проходят по повтору — их сюда пускать нельзя даже если рядом
    # в тексте попалось слово из списка.
    for needle, reason in _UNRECOVERABLE:
        if needle in blob:
            return reason
    return ""
