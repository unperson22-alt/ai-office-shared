...
def check_office_token(request) -> bool:
    """
    True если запрос несёт валидный office-токен ЛИБО токен ещё не сконфигурирован.

    ⚠️ ЭТО НЕ АУТЕНТИФИКАЦИЯ. При пустом OFFICE_RPC_TOKEN функция возвращает True
    ВСЕМ — она отвечает на вопрос «блокировать ли выкат», а не «свой ли это».
    Годится только там, где отказ включается отдельным рычагом
    (OFFICE_RPC_STRICT), то есть внутри office_auth_middleware.

    НЕ используй её как условие пропуска в собственном гейте: `if
    check_office_token(request): return True` в Фазе A открывает вход всем. Для
    гейта есть has_office_token() — она при невыставленном секрете говорит «нет».

    Предупреждение написано по факту: 26.09.2026 ровно эта строка открыла у Эллис
    /send, /generate, /mention и /send_scheduled, а /send отправляет сообщение от
    её имени в произвольный chat_id. Прежний докстринг тоже описывал это
    поведение — в скобках, как деталь раскатки, — и был прочитан как разрешение.
    Урок #145.
    """
...
def has_office_token(request) -> bool:
    """
    True только если office-токен НАСТРОЕН и запрос несёт именно его.

    Отличие от check_office_token одно: пустой OFFICE_RPC_TOKEN здесь — «нет».
    Именно это нужно боту, который складывает свой гейт из нескольких учёток:
    разрешительность переходного режима не должна открывать дверь, которая без
    неё была закрыта.

    Пока OFFICE_RPC_TOKEN не выставлен, функция отказывает всегда — и это верно:
    в Фазе A office-учётки просто не существует, и пропускать по ней нечего.
    """
    return bool(OFFICE_RPC_TOKEN) and check_office_token(request)
